...
//	//  Open_Window(0,0,500,500);
//	Write_Cmd(CMD_SetPixel);
//	len=2*((uint16_t)(a[2]<<8)+a[3])*((uint16_t)(a[4]<<8)+a[5]);   //计算出图像所占的字节数
//	while(i<(len+8))
//	{
////从图像数组的第9位開始递增7/16位总线，须要一次发送2个字节的数据
//		temp=(uint16_t)(a[i]<<8)+a[i+1];//将取出的16位像素数据送入显示区
//        Write_Data(temp);
//        i=i+2;
//	}
//}
...
//LCD_SCAN_MODE=ucOption;//根据模式更新XY方向的像素宽度
//if(ucOption%2==0)
//{
...
//	LCD_Y_LENGTH= ILI9341LESS_PIXEL;
//}
////0x36命令参数的高3位可用于设置GRAM扫描方向
//Write_Cmd(0x36);
//Write_Data(0x08 |(ucOption<<5));//根据ucOption的值设置LCD参数，共0-7种模式
//Write_Cmd(CMD_SetCoordinateX);
///* x 起始坐标高8位 */
//Write_Data( 0x00);
//Write_Data( 0x00 );
///* x 起始坐标低8位 */
//
//Write_Data(((LCD_X_LENGTH-1)>>8)&0xFF);/* x结束坐标高8位 */
//Write_Data((LCD_X_LENGTH-1)&0xFF );
///* x 结束坐标低8位 */
//Write_Cmd(CMD_SetCoordinateY);
///* y 起始坐标高8位 */
//Write_Data (0x00);
//Write_Data ( 0x00 );
///* y 起始坐标低8位 */
//
//Write_Data(((LCD_Y_LENGTH-1)>>8)&0xFF);/* x结束坐标高8位 */
//Write_Data((LCD_Y_LENGTH-1)&0xFF );
///* y 结束坐标低8位 */
///* write gram start */
//Write_Cmd(CMD_SetPixel);
//}
...
//	for (k = 0; k < 150; k++) {
//		for (t = 0; t < 150; t++) {
//			temp = gImage_Yagushou[k * 150 + t];
//			POINT_COLOR=gImage_Yagushou[k * 150 + t];
//			//if (temp & 0x80)
//			//if (temp)
//				LCD_Fast_DrawPoint(x, y, POINT_COLOR);
//			//else if (mode == 0)
...
//		}
//	}
//	POINT_COLOR=POINT_COLOR1;
//}


void LCD_ShowImage1_1(uint16_t x, uint16_t y, uint16_t level) {
	uint8_t temp, t, k;
	uint16_t y0 = y;
	int mode = 0;
	int size = 67;
	for (k = 0; k < 67; k++) {
		for (t = 0; t < 67; t++) {
			if (level==1){
				temp = gImage_P1_1[k * 67 + t];
			}
			else if (level==2){
				temp = gImage_P1_2[k * 67 + t];

			}else if (level==3){
				temp = gImage_P1_3[k * 67 + t];

			}else if (level==4){
				temp = gImage_P1_4[k * 67 + t];

			}else if (level==5){
				temp = gImage_P1_5[k * 67 + t];
...
void LCD_ShowImage1_2(uint16_t x, uint16_t y, uint16_t level) {
	uint8_t temp, t, k;
	uint16_t y0 = y;
	int mode = 0;
	int size = 67;
	for (k = 0; k < 67; k++) {
		for (t = 0; t < 67; t++) {
			if (level==1){
							temp = gImage_P2_1[k * 67 + t];
						}
						else if (level==2){
							temp = gImage_P2_2[k * 67 + t];

						}else if (level==3){
							temp = gImage_P2_3[k * 67 + t];

						}else if (level==4){
							temp = gImage_P2_4[k * 67 + t];

						}else if (level==5){
							temp = gImage_P2_5[k * 67 + t];
...
void LCD_ShowImage1_3(uint16_t x, uint16_t y, uint16_t level) {
	uint8_t temp, t, k;
	uint16_t y0 = y;
	int mode = 0;
	int size = 67;
	for (k = 0; k < 67; k++) {
		for (t = 0; t < 67; t++) {
			if (level==1){
							temp = gImage_P3_1[k * 67 + t];
						}
						else if (level==2){
							temp = gImage_P3_2[k * 67 + t];

						}else if (level==3){
							temp = gImage_P3_3[k * 67 + t];

						}else if (level==4){
							temp = gImage_P3_4[k * 67 + t];

						}else if (level==5){
							temp = gImage_P3_5[k * 67 + t];
...
void LCD_ShowImage2_1(uint16_t x, uint16_t y, uint16_t level) {
	uint8_t temp, t, k;
	uint16_t y0 = y;
	int mode = 0;
	int size = 67;
...
void LCD_ShowImage2_2(uint16_t x, uint16_t y, uint16_t level) {
	uint8_t temp, t, k;
	uint16_t y0 = y;
	int mode = 0;
	int size = 67;
...
void LCD_ShowImage2_3(uint16_t x, uint16_t y, uint16_t level) {
	uint8_t temp, t, k;
	uint16_t y0 = y;
	int mode = 0;
	int size = 67;
...
void LCD_ShowImage0(uint16_t x, uint16_t y) {
	uint8_t temp, t, k;
	uint16_t y0 = y;
	int mode = 0;
	int size = 67;
...
void LCD_ShowImage3(uint16_t x, uint16_t y,uint16_t type) {
	uint8_t temp, t, k;
	uint16_t y0 = y;
	int mode = 0;
	int size = 30;
	for (k = 0; k < 30; k++) {
		for (t = 0; t < 30; t++) {
			if (type==1){
				temp = gImage_ATK[k * 30 + t];
			}else if (type==2){
				temp = gImage_DEF[k * 30 + t];
			}else if (type==3){
				temp = gImage_SK[k * 30 + t];
			}else if (type==4){
				temp = gImage_ATK2[k * 30 + t];
			}
